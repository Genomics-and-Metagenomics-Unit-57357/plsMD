...
|---|---|---|
| `<preprocessing_output>/unicycler_fasta/` | Contains the unicycler  FASTA assemblies provided as input. | `plsMD --preprocessing` |
| `<preprocessing_output>/<fasta_file>_plasmid.txt` | ABRICATE output files for each input FASTA file, detailing identified antimicrobial resistance genes. | `plsMD --preprocessing` |
| `<preprocessing_output>/<fasta_file>_PLSDB.txt` | BLASTn output files for each input FASTA file, showing matches against the PLSDB database. | `plsMD --preprocessing` |
| `<preprocessing_output>/plasmid_list.txt` | A list of identified plasmid replicons. | `plsMD --preprocessing` |
| `<processing_output>/gene_directories/` | Contains the gene directories used in the phylogenetic analysis | `plsMD --processing` |
| `<processing_output>/plasmid_files/` | Contains processed plasmid contig files. | `plsMD --processing` |
| `<processing_output>/nonplasmid_files/` | Contains processed non-plasmid contig files. | `plsMD --processing` |
| `<processing_output>/Processed_ColContigs.txt` | Combined output file for processed Col and Inc plasmids. | `plsMD --processing` |
| `<annotation_output>` | Contains Output from AMRfinder, abricate (vfdb-plasmidfinder) and blastn (IS) for plasmid and nonplasmid sequences. | `plsMD --annotation` |
| `<phylogenetics_output>/phylogenetic_tree/` | Contains the output of the MSA and phylogenetic analysis. | `plsMD --phylogenetics` |
----------
## Citations
