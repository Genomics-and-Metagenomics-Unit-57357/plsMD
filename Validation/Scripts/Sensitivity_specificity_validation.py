...
def step_q_prec_cov(input_dir, output_dir):
    print("\n" + "="*60)
    print(" STEP 3 – Query aligned bases (q_prec_cov)")
    print("="*60)
    ...
    for filename in sorted(os.listdir(input_dir)):
        ...
        for qseqid in df["qseqid"].unique():
            qseqid_rows = df[df["qseqid"] == qseqid]

            max_end  = int(qseqid_rows["qend"].max())
            coverage = np.zeros(max_end + 1, dtype=bool)

            for _, row in qseqid_rows.iterrows():
                coverage[int(row["qstart"]):int(row["qend"]) + 1] = True

            qseqid_aligned[qseqid] = int(coverage.sum())

            df.loc[df["qseqid"] == qseqid, "qsseqid_aligned"] = qseqid_aligned[qseqid]

        total_qsseqid_aligned = sum(qseqid_aligned.values())
        df["sum_qsseqid_aligned"] = total_qsseqid_aligned
