...
- **Single Modality** – Reconstructed plasmids are annotated for AMR genes, VF genes, and ISs.
...
- Both outputs are combined, with redundant genes removed, producing `Sample_plasmid.txt`.
- All contigs are reversed, `_R` is appended to the contig name, and merged with the original contigs into `sample_merged.fasta`.
